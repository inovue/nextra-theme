...
function HeadingLink({ tag: Tag, children, id, ...props }: ComponentProps<'h2'> & { tag: `h${2 | 3 | 4 | 5 | 6}` }): ReactElement {
  return (
    <Tag className={`subheading-${Tag}`} {...props}>
      {children}
      <span className="nx-absolute -nx-mt-20" id={id} />
      <a href={id && `#${id}`} className="subheading-anchor" aria-label="Permalink for this section" />
    </Tag>
  )
}
...
const useComponents = (): Components => {
  ...
  return {
    ...
    th: Th,
    td: Td,
    table: props => <div className="nx-not-prose"><Table className="" {...props} /></div>,
    code: Code,
    ...config.components
  }
}

export const MDXContainer = ({ children }: { children: ReactNode }): ReactElement => {
  return (
